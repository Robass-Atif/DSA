...
public:
    ...
    void copy(string name, string extension, string directoryName)
    {
        ...
        if (fileToCopy != nullptr)
        {
            ...
            //     }
            // }
            paste(fileToCopy, directoryName, currentDirectory);
        }
    }
    // paste recursive function
    void paste(File *fileToCopy, string directoryName, Directory *currentDirectory)
    {
        // base condtion
        for (auto dir : currentDirectory->directories)
        {
            ...
            {
                paste(fileToCopy, directoryName, dir);
            }
        }
    }
    ...
    void dir()
    {
        ...
        cout << "Total Files: " << row << endl;
        cout << "Total Directories: " << col << endl;
    }
    ...
    void loadTree()
    {
        ...
        while (getline(myfile, line))
        {
            ...
            Directory *dir = new Directory(name, parent, path);
            if (parent != nullptr)
            {
                parent->directories.push_back(dir);
            }
        }
        myfile.close();
        ifstream myfile2;
        ...
        while (getline(myfile2, line2))
        {
            ...
            Directory *dir = findDirectory(root, directoryName);
            File *file = new File(name, extension, content);
            dir->files.push_back(file);
        }
        myfile2.close();  

    }
    // find directory recursive function
    Directory *findDirectory(Directory *currentDirectory, string name)
    {
        ...
        for (auto dir : currentDirectory->directories)
        {
            Directory *found = findDirectory(dir, name);
            if (found != nullptr)
            {
                return found;
            }
    ...
    void SAVE()
    {
        ...
        for (auto fil : root->files)
        {
            myfile2 << fil->name << "," << fil->extension<<"," <<currentDirectory->name<<","<<fil->content << endl;
        }
        for (auto dir : root->directories)
        {
            myfile << dir->name << ",";
            myfile << dir->path << ",";
            myfile << dir->parent->name << endl;
            save(dir);
        }
        myfile.close();
        myfile2.close();
    }
    void save(Directory* currentDirectory)
    {

        if(currentDirectory->directories.size()==0 && currentDirectory->files.size()==0)
        {
            return;
        }
        if(currentDirectory->directories.size()!=0 && currentDirectory->files.size()!=0)
        {
            ...
            {
                ofstream myfile;
                myfile.open("directories.txt",ios::app);
                myfile << dir->name << ",";
                myfile << dir->path << ",";
                myfile << dir->parent->name<< endl;
                myfile.close();
            ...
            for(auto fil:currentDirectory->files)
            {
                ofstream myfile2;
                myfile2.open("files.txt",ios::app);
                myfile2 << fil->name << "," << fil->extension<<"," << currentDirectory->name<<","<<fil->content<< endl;
                myfile2.close();
            }
        }
        else if(currentDirectory->directories.size()==0 && currentDirectory->files.size()!=0)
        {
            for(auto fil:currentDirectory->files)
            {
                ofstream myfile2;
    ...
    void userInput()
    {
        ...
        while (true)
        {
            ...
            {
                // format print <filename>.<extension>
                string name = args.substr(0, args.find("."));
                string extension = args.substr(args.find(".") + 1);
